...
A complete, organized inventory of every feature in the Hozio Pro plugin (v4.21.0), with an explanation of how the modules connect and how the whole system works. Covers all 37 PHP files (~44,000 lines) plus the JS/CSS assets — every feature, shortcode, hook, REST endpoint, WP-CLI command, cron job, admin page, and `wp_option`.

> Last full refresh: 4.21.0. Earlier sections were written against 4.12.12 and have been corrected where the code has since changed; when this file and the code disagree, the code is right — fix this file.
...
- `update_option` names are compared case-insensitively (and without trailing spaces) by MySQL. Any allow/deny check on option names must normalise first — the Hub's `update_option` accepts only `^hozio_[a-z0-9_-]+$`.
