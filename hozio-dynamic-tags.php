...
// Update holds and the site-wide update freeze. Loaded before the updater, the auto-update
// module and the Hub files, which all consult it.
require_once plugin_dir_path( __FILE__ ) . 'includes/update-holds.php';
...
// `wp hozio ...` — the Hozio Studio Orchestrator's interface. Only ever loaded under WP-CLI.
...
register_deactivation_hook(__FILE__, function() {
    ...
    if (function_exists('hozio_sug_deactivate')) {
        hozio_sug_deactivate();
    }
});
...
add_action('init', function() {
    ...
    if (version_compare($last_version, HOZIO_VERSION, '<')) {
        ...
        // First run of a newer release: lift a Hub downgrade's pin on Hozio Pro that this
        // release has outgrown (includes/update-holds.php). Hub-placed holds only.
        if (function_exists('hozio_update_hold_release_stale_hub_pin')) {
            hozio_update_hold_release_stale_hub_pin(HOZIO_VERSION);
        }
